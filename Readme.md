...
## クラス
簡単に言うとオブジェクトの設計図
関数は小文字からはじめる。クラスは大文字から始める。区別あるといいかも！
### オブジェクト:データとメソッドが一緒になったもの（単なるデータではなくどのような振る舞いをすればいいかしっている）
### インスタンス：d = MyClass() クラスからオブジェクトを作成したもの(オブジェクトと似たつくり、メソッド持ってたり)
### アトリビュート：i.value = 5 などインスタンスにデータを持たせることができる（インスタンスごとに存在）
いちいち各インスタンスにアトリビュートするのはめんどくさい！あらかじめ代入したい
・・・
インスタンスが作られたときに自動的に呼ばれる初期化メソッドを使おう!!!!!
クラス内のメソッド：def文をかく。引数のセルフはインスタンス自体のこと
## 初期化メソッド：__init__()　名前はこれじゃないとダメ。インスタンス生成時に自動的に呼ばれる
    ※他のメソッドも第1引数はself

## カプセル化
メソッドやアトリビュートを外部から利用できなくさせること
アトリビュート名やメソッド名の先頭に_をつける
1つ_：暗黙の了解のようなもの？（書き換えちゃいけないんだなぁ的な）
2つ__:外部からアクセスができなくなる
疑似的な隠ぺいにすぎない
